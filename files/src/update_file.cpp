...
std::tuple<std::string, int, float> take_input()
{
       ...
       std::cout << "\n\t Now tell how old the person is: ";
       std::cin >> age;
       if(age < 0)
       {
         throw std::system_error(errno,
...
{
    std::string success("\n\n\t :::: New data successfully stored in file! :::: ");
    std::fstream file;
    std::stringstream contents;

    std::cout << "\n\t  Appending new data to file..." << std::endl;

    file.open("register.txt", std::ios::out | std::ios::app);
    std::string line("\n\t----------------------------------------\n");
